 ...
 *   Left sidebar  : Rounds / Heatmap / Multi-Round tabs
 *   Centre        : RadarViewer + PlaybackControls
 *   Right sidebar : Players list / Live Info tabs
 */
            ...
            <span className={styles.roundMeta}>
              ...
              {roundInfo.winner_team && (
                <span
                  ...
                  style={{
                    color: roundInfo.winner_team === 'CT' ? TEAM_COLORS.CT : TEAM_COLORS.T,
                  }}
                >
      ...
      <div className={styles.body}>
        ...
        <aside className={styles.sidebar}>
          <div className={styles.tabBar}>
            ...
            <button
              className={`${styles.tab} ${sideTab === 'multi' ? styles.activeTab : ''}`}
              onClick={() => setSideTab('multi')}
            >
              Multi
            </button>
          </div>
        ...
        {/* Right panel: players / live info */}
        <aside className={styles.rightPanel}>
          <div className={styles.rightTabBar}>
            <button
              className={`${styles.rightTab} ${rightTab === 'players' ? styles.rightTabActive : ''}`}
              onClick={() => setRightTab('players')}
            >
              Players
            </button>
            <button
              className={`${styles.rightTab} ${rightTab === 'info' ? styles.rightTabActive : ''}`}
          ...
          {rightTab === 'players' && (
            <>
              ...
              <div className={styles.playerList}>
                ...
                {players.length === 0 && <p className={styles.emptyPlayers}>—</p>}
              </div>
            </>
          )}
